...
按住热键呼出切换器，每按一次切换到下一个应用，松开即以 `open -a`（= 点击 Dock 图标）打开/激活选中应用 —— 无需移动鼠标，连「进程在但窗口全关」的应用也能直接打开并开出新窗口。
...
- **`open -a` 单纯激活** — 松键单纯执行 `open -a <应用真实路径>`（即 `open -a xxx.app`，= 点击 Dock 图标），一条指令覆盖所有状态：无窗应用（进程在、窗口全关）触发系统 reopen 事件开出窗口；任意桌面的应用都被激活（按系统「切换到某个应用程序时，转到该应用已打开窗口的 Space」设置切到窗口所在桌面，默认开）；已退出的应用重新启动。所有已打开应用均在列表中（含无窗/⌘H 隐藏），唯一移除条件是进程退出
...
├── Services/     # 快捷键监听、应用枚举、激活（open -a）、桌面索引（CGS 查询/自检）
├── UI/           # 面板、设置窗口、托盘、视图
├── Effects/      # 玻璃主题与高亮边框
└── Models/       # 全局状态模型
```
