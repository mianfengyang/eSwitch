...
- **跨虚拟桌面切换** —— 目标应用在其他桌面时，用系统原生 `activateAllWindows`（=Cmd+Tab 语义）把窗口拉回当前 Space，不跳转任何桌面
...
- macOS 12.0 及以上（跨桌面激活使用系统原生 `activateAllWindows`，老系统自动退化为普通激活）
...
- 跨空间激活依赖系统原生 `activateAllWindows`（等价于手动 Cmd+Tab）；若手动 Cmd+Tab 也无法把窗口拉回，属于系统层面限制（如独立全屏 Space），而非 eSwitch 问题
